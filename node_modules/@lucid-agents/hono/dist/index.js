...
import { resolvePrice, validatePaymentsConfig, resolvePayTo, extractSenderDomain, evaluateSender, extractPayerAddress, parsePriceAmount, findMostSpecificIncomingLimit, createFacilitatorAuthHeaders } from '@lucid-agents/payments';

// src/app.ts
var DEFAULT_SCHEME = "exact";
var DEFAULT_SCHEMES = [
  {
    network: "eip155:*",
...
function withPaymentHeader(c, paymentHeader) {
      ...
      if (prop === "header") {
        return (name) => {
          ...
          if (name.toLowerCase() === "x-payment") {
            return requestWithPayment.headers.get("X-PAYMENT") ?? void 0;
          }
...
function addFacilitatorAuth(facilitator, token) {
...
function withPayments({
  ...
  const baseFacilitator = facilitator ?? { url: payments.facilitatorUrl };
...
async function createAgentApp(runtime, opts) {
  ...
  app.get(
    ...
    (c) => runtime.handlers.oasf ? runtime.handlers.oasf(c.req.raw) : c.json(
      {
        error: {
          code: "not_found",
